...
class UserView(ABC):
    """Абстрактний базовий клас для користувацьких уявлень (views)."""
...
class ConsoleView(UserView):
    """Реалізація представлення для консольного інтерфейсу."""
    ...
    def show_help(self):
        commands = [
            ...
            "phone <name> — показати номери контакту",
...
def main():
    ...
    view.display_message("Ласкаво просимо до вашого асистента! 😊")

    while True:
        user_input = view.get_input("Введіть команду: ").strip()
        if not user_input:
            view.display_message("Будь ласка, введіть команду.")
            continue
        ...
        if command in ["close", "exit"]:
            save_data(book)
            view.display_message("До побачення! 👋")
            break
        elif command == "hello":
            view.display_message("Привіт! Чим можу допомогти? 🖐️")
        elif command == "help":
            view.show_help()
        elif command == "add":
            ...
            view.display_message("Невідома команда. Напишіть 'help', щоб побачити список команд.")
